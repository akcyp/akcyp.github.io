(function(){function r(e,n,t){function o(i,f){if(!n[i]){if(!e[i]){var c="function"==typeof require&&require;if(!f&&c)return c(i,!0);if(u)return u(i,!0);var a=new Error("Cannot find module '"+i+"'");throw a.code="MODULE_NOT_FOUND",a}var p=n[i]={exports:{}};e[i][0].call(p.exports,function(r){var n=e[i][1][r];return o(n||r)},p,p.exports,r,e,n,t)}return n[i].exports}for(var u="function"==typeof require&&require,i=0;i<t.length;i++)o(t[i]);return o}return r})()({1:[function(require,module,exports){
  ...
  var AbstractMove = (function (_super) {
      __extends(AbstractMove, _super);
      function AbstractMove(game, source, target, capture, castling) {
          var _this = _super.call(this, source, target, capture) || this;
          _this.game = game;
          _this.source = source;
          _this.target = target;
          _this.capture = capture;
          _this.castling = castling;
          _this.abstractGame = null;
          _this.promotion = false;
          if (_this.game.select(source).pieceName === 'Pawn' && ['1', '8'].includes(target.charAt(1))) {
              _this.promotion = true;
          }
          if (game.abstract === false) {
              _this.abstractGame = new Game_1.Game(_this.game.toFen(), true);
              _this.abstractGame.takeMove(source, target, new AbstractMove(_this.abstractGame, source, target, capture));
          }
          return _this;
      }
      return AbstractMove;
  ...
  },{"./Game":2,"./Move":3}],2:[function(require,module,exports){
  "use strict";
  var __extends = (this && this.__extends) || (function () {
      var extendStatics = function (d, b) {
          extendStatics = Object.setPrototypeOf ||
  ...
  Object.defineProperty(exports, "__esModule", { value: true });
  var AbstractMove_1 = require("./AbstractMove");
  var Move_1 = require("./Move");
  var Piece_1 = require("./Piece");
  var boardIterator_1 = require("./helpers/boardIterator");
  var EventEmitter_1 = require("./helpers/EventEmitter");
  var fenParser_1 = require("./helpers/fenParser");
  var Point_1 = require("./helpers/Point");
  var Game = (function (_super) {
      __extends(Game, _super);
      function Game(fen, abstract) {
          ...
          _this.possibleCastlings = {
              b: { kingside: false, queenside: false },
              w: { kingside: false, queenside: false },
          };
          _this.turn = 'w';
          _this.lastCatchNumber = Infinity;
          _this.moveNumber = 0;
          _this.enpassant = null;
          _this.board = [];
          _this.pieces = new Set();
          _this.reset();
          _this.parseFen(fen);
          _this.emit('start');
          return _this;
      ...
      Game.prototype.reset = function () {
          this.board = Array.from({ length: 8 }, function () { return Array.from({ length: 8 }, function () { return null; }); });
          this.pieces = new Set();
      };
      ...
      Game.prototype.invertColor = function (color) {
          if (color === 'b') {
              return 'w';
          }
          return 'b';
      };
      Game.prototype.select = function (pos) {
          var _a = new Point_1.Point(pos), x = _a.x, y = _a.y;
          return this.getPiece(x, y);
      };
      Game.prototype.getPositionAttackingMoves = function (pos, color) {
          var pieces = __spread(this.pieces).filter(function (piece) { return piece.color === color; });
          var attackMoves = pieces.map(function (piece) { return piece.getLegalMoves(); }).reduce(function (prev, now) { return prev.concat(now); }, []);
          return attackMoves.filter(function (move) {
              return pos === move.target;
          });
      };
      Game.prototype.getKingAttackingMoves = function (color) {
          var _this = this;
          var kingsPos = __spread(this.pieces).filter(function (piece) { return piece.pieceName === 'King' && piece.color === color; })
              .map(function (king) { return king.pos; });
          return kingsPos
              .map(function (pos) {
              return _this.getPositionAttackingMoves(pos, _this.invertColor(color));
          })
              .reduce(function (prev, now) { return prev.concat(now); }, []);
      };
      Game.prototype.isCheck = function (color) {
      ...
      Game.prototype.isDraw = function () {
          ...
          return (!this.isCheck(this.turn) &&
              __spread(this.pieces).filter(function (piece) { return piece.color === _this.turn; })
                  .every(function (piece) { return piece.getPossibleMoves().length === 0; }));
      };
      ...
      Game.prototype.takeMove = function (source, target, abstractMove) {
          var _a = new Point_1.Point(target), x = _a.x, y = _a.y;
          var piece = this.select(source);
          if (!(piece instanceof Piece_1.Piece)) {
              throw new Error('Invalid move');
          }
          var move;
          if (abstractMove instanceof AbstractMove_1.AbstractMove) {
              move = abstractMove;
          }
          else {
              move = piece.getPossibleMoves().find(function (m) { return m.x === x && m.y === y; });
              if (!(move instanceof AbstractMove_1.AbstractMove)) {
                  throw new Error("Cannot move to " + target);
              }
          }
          if (piece.pieceName === 'Pawn' && Math.abs(move.y - piece.position.y) === 2) {
              var enPassantPos = Point_1.Point.getNotation(piece.position.x, (piece.position.y + move.y) / 2);
              this.enpassant = new Point_1.Point(enPassantPos, this.turn);
          }
          else {
              this.enpassant = null;
          }
          if (move.capture) {
              var captured = this.select(move.capture);
              captured.destroy();
          ...
          var castlingPiece = move.castling instanceof Move_1.Move ? this.select(move.castling.source) : null;
          piece.setPosition(x, y);
          if (castlingPiece instanceof Piece_1.Piece) {
              castlingPiece.setPosition(move.castling.targetPosition.x, move.castling.targetPosition.y);
          }
          this.lastCatchNumber += 1;
          this.moveNumber += 1;
      ...
      Game.prototype.emitMove = function (move) {
          if (move.capture) {
      ...
      Game.prototype.parseFen = function (fen) {
          var _this = this;
          this.reset();
          var _a = fenParser_1.parseFen(fen), pieces = _a.pieces, possibleCastlings = _a.possibleCastlings, turn = _a.turn, enpassant = _a.enpassant, lastCatchNumber = _a.lastCatchNumber, moveNumber = _a.moveNumber;
          this.possibleCastlings = possibleCastlings;
          this.turn = turn;
          this.lastCatchNumber = lastCatchNumber;
          this.moveNumber = moveNumber;
          pieces.forEach(function (piece) {
              var pos = Point_1.Point.getNotation(piece.x, piece.y);
              var p = new Piece_1.Piece(pos, piece.color, piece.token, _this);
              _this.board[piece.y - 1][piece.x - 1] = p;
              _this.pieces.add(p);
          });
          if (enpassant === '-') {
              this.enpassant = null;
              ...
              var position = new Point_1.Point(enpassant, this.turn === 'w' ? 'b' : 'w');
      ...
      Game.prototype.toFen = function () {
          var fen = this.board
              .map(function (row) {
              return row
                  .map(function (piece) { return (piece === null ? '#' : piece.token); })
                  .join('')
                  .replace(/#+/g, function (match) { return match.length.toString(); });
          })
              .reverse()
              .join('/');
          fen += ' ' + this.turn + ' ';
          var castling = Object.entries(this.possibleCastlings.w)
              .filter(function (_a) {
      ...
      Game.prototype.iterator = function (pos, color, mode, limit) {
          var e_1, _a, e_2, _b;
          if (limit === void 0) { limit = Infinity; }
          var _c = new Point_1.Point(pos), x = _c.x, y = _c.y;
          var iterators = [];
          if (mode !== 'line') {
              iterators.push.apply(iterators, __spread(boardIterator_1.boardIteratorsGenerator(x, y, mode)));
          }
          else {
              iterators.push.apply(iterators, __spread(boardIterator_1.boardIteratorsGenerator(x, y, 'straight')));
              iterators.push.apply(iterators, __spread(boardIterator_1.boardIteratorsGenerator(x, y, 'diagonal')));
          }
          var moves = [];
          try {
              for (var iterators_1 = __values(iterators), iterators_1_1 = iterators_1.next(); !iterators_1_1.done; iterators_1_1 = iterators_1.next()) {
                  var iterator = iterators_1_1.value;
                  try {
                      for (var _d = (e_2 = void 0, __values(__spread(iterator).entries())), _e = _d.next(); !_e.done; _e = _d.next()) {
                          var _f = __read(_e.value, 2), i = _f[0], _g = _f[1], x_1 = _g.x, y_1 = _g.y;
                          if (i >= limit) {
                              break;
                          }
                          var square = this.getPiece(x_1, y_1);
                          if (square instanceof Piece_1.Piece) {
                              if (square.color !== color) {
                                  moves.push(new Move_1.Move(pos, Point_1.Point.getNotation(x_1, y_1), square.pos));
                              }
                              break;
                          }
                          else {
                              moves.push(new Move_1.Move(pos, Point_1.Point.getNotation(x_1, y_1)));
                          }
                      }
                  }
  ...
  },{"./AbstractMove":1,"./Move":3,"./Piece":4,"./helpers/EventEmitter":5,"./helpers/Point":6,"./helpers/boardIterator":7,"./helpers/fenParser":8}],3:[function(require,module,exports){
  "use strict";
  Object.defineProperty(exports, "__esModule", { value: true });
  var Point_1 = require("./helpers/Point");
  var Move = (function () {
      function Move(source, target, capture) {
          this.source = source;
          this.target = target;
          this.capture = capture;
          this.sourcePosition = new Point_1.Point(source);
          this.targetPosition = new Point_1.Point(target);
      }
  ...
  },{"./helpers/Point":6}],4:[function(require,module,exports){
  "use strict";
  var __read = (this && this.__read) || function (o, n) {
      var m = typeof Symbol === "function" && o[Symbol.iterator];
      if (!m) return o;
  ...
  var Piece = (function () {
      function Piece(pos, color, token, game) {
          this.color = color;
          this.token = token;
          this.game = game;
          this.startingPosition = new Point_1.Point(pos);
          this.position = new Point_1.Point(pos);
      }
      Object.defineProperty(Piece.prototype, "pieceName", {
          get: function () {
              return tokenParser[this.token.toLowerCase()];
          },
          enumerable: true,
          configurable: true
      });
      Object.defineProperty(Piece.prototype, "pos", {
          ...
          set: function (value) {
              this.position = new Point_1.Point(value);
          },
          enumerable: true,
          configurable: true
      });
      Piece.prototype.move = function (target) {
          return this.game.takeMove(this.pos, target);
      };
      Piece.prototype.getLegalMoves = function () {
          var _this = this;
          return this.getMoves().map(function (move) {
              return new AbstractMove_1.AbstractMove(_this.game, move.source, move.target, move.capture);
          });
      };
      Piece.prototype.getMoves = function () {
          var _this = this;
          if (this.pieceName === 'Pawn') {
              var v_1 = this.color === 'w' ? 1 : -1;
              var isStarter = (this.color === 'w' ? 2 : 7) === this.position.y;
              var movesUp = this.game.iterator(this.pos, this.color, 'straight', isStarter ? 2 : 1).filter(function (move) {
                  return (move.x === _this.position.x &&
                      v_1 * (move.y - _this.position.y) > 0 &&
                      !move.capture);
              });
              var captureMoves = this.game.iterator(this.pos, this.color, 'diagonal', 1).filter(function (move) {
                  return (v_1 * (move.y - _this.position.y) > 0 && move.capture);
              });
              var moves = __spread(movesUp, captureMoves);
              if (this.game.enpassant &&
                  this.color !== this.game.enpassant.color &&
                  Math.abs(this.position.x - this.game.enpassant.x) === 1 &&
                  v_1 * (this.game.enpassant.y - this.position.y) === 1) {
                  var enpassantPiece = this.game.getPiece(this.game.enpassant.x, this.position.y);
                  moves.push(new Move_1.Move(this.pos, this.game.enpassant.pos, enpassantPiece.pos));
              }
              return moves;
          }
          ...
          if (this.pieceName === 'King') {
              var moves = this.game.iterator(this.pos, this.color, 'line', 1);
              return moves;
          }
          return [];
      };
      Piece.prototype.getPossibleMoves = function () {
          ...
          if (this.pieceName === 'King') {
              var isCheck = this.game.isCheck(this.color);
              if (!isCheck) {
                  if (this.game.possibleCastlings[this.color].kingside) {
                      if (!(this.game.getPiece(this.position.x + 1, this.position.y) instanceof Piece) &&
                          !(this.game.getPiece(this.position.x + 2, this.position.y) instanceof Piece) &&
                          this.game.getPositionAttackingMoves(Point_1.Point.getNotation(this.position.x + 1, this.position.y), this.game.invertColor(this.color)).length === 0) {
                          var castlingMove = new Move_1.Move(Point_1.Point.getNotation(8, this.position.y), Point_1.Point.getNotation(this.position.x + 1, this.position.y));
                          var move = new AbstractMove_1.AbstractMove(this.game, this.pos, Point_1.Point.getNotation(this.position.x + 2, this.position.y), undefined, castlingMove);
                          moves.push(move);
                      }
                  }
                  if (this.game.possibleCastlings[this.color].queenside) {
                      if (!(this.game.getPiece(this.position.x - 1, this.position.y) instanceof Piece) &&
                          ...
                          this.game.getPositionAttackingMoves(Point_1.Point.getNotation(this.position.x - 1, this.position.y), this.game.invertColor(this.color)).length === 0) {
                          var castlingMove = new Move_1.Move(Point_1.Point.getNotation(1, this.position.y), Point_1.Point.getNotation(this.position.x - 1, this.position.y));
                          var move = new AbstractMove_1.AbstractMove(this.game, this.pos, Point_1.Point.getNotation(this.position.x - 2, this.position.y), undefined, castlingMove);
                          moves.push(move);
                      }
                  }
          ...
          return moves.filter(function (move) {
              return move.abstractGame.isCheck(_this.color) === false;
          });
      };
      Piece.prototype.destroy = function () {
          var e_1, _a;
          try {
              for (var _b = __values(this.game.board), _c = _b.next(); !_c.done; _c = _b.next()) {
                  var row = _c.value;
      ...
      Piece.prototype.setPosition = function (x, y) {
          ...
          this.pos = Point_1.Point.getNotation(x, y);
      };
      return Piece;
  }());
  exports.Piece = Piece;
  
  },{"./AbstractMove":1,"./Move":3,"./helpers/Point":6}],5:[function(require,module,exports){
  "use strict";
  var __values = (this && this.__values) || function(o) {
      var s = typeof Symbol === "function" && Symbol.iterator, m = s && o[s], i = 0;
      if (m) return m.call(o);
  ...
  var EventEmitter = (function () {
      ...
      EventEmitter.prototype.off = function (key, listener) {
          if (this._events[key]) {
              if (listener instanceof Function) {
                  var idx = this._events[key].indexOf(listener);
                  if (idx !== -1) {
      ...
      EventEmitter.prototype.emit = function (key) {
          ...
          if (this._events[key]) {
              var listeners = this._events[key].slice();
              try {
  ...
  var Point = (function () {
      ...
      Point.getNotation = function (x, y) {
          return 'abcdefgh'.split('')[x - 1] + y;
      };
      Point.validate = function (position) {
          return /^[abcdefgh][12345678]$/i.test(position);
      };
      Point.prototype.getNotation = function () {
  ...
  Object.defineProperty(exports, "__esModule", { value: true });
  function isNotBeyondTheBorder(x, y) {
      return x >= 1 && x <= 8 && y >= 1 && y <= 8;
  }
  function lineIterator(_a, vx, vy) {
      var x = _a.x, y = _a.y;
      return __generator(this, function (_b) {
          switch (_b.label) {
              case 0:
                  if (!isNotBeyondTheBorder(x + vx, y + vy)) return [3, 2];
                  x += vx;
                  y += vy;
                  return [4, { x: x, y: y }];
              case 1:
  ...
  function LshapeIterator(_a, vx, vy) {
      var x = _a.x, y = _a.y;
      return __generator(this, function (_b) {
          switch (_b.label) {
              case 0:
  ...
  function boardIteratorsGenerator(x, y, mode) {
      var e_1, _a;
      var iterators = [];
      if (mode === 'straight') {
          iterators.push(lineIterator({ x: x, y: y }, 0, 1));
          iterators.push(lineIterator({ x: x, y: y }, 0, -1));
          iterators.push(lineIterator({ x: x, y: y }, 1, 0));
          iterators.push(lineIterator({ x: x, y: y }, -1, 0));
      }
      if (mode === 'diagonal') {
          iterators.push(lineIterator({ x: x, y: y }, 1, 1));
          iterators.push(lineIterator({ x: x, y: y }, 1, -1));
          iterators.push(lineIterator({ x: x, y: y }, -1, 1));
          iterators.push(lineIterator({ x: x, y: y }, -1, -1));
      }
      if (mode === 'Lshape') {
          ...
          try {
              for (var Lvectors_1 = __values(Lvectors), Lvectors_1_1 = Lvectors_1.next(); !Lvectors_1_1.done; Lvectors_1_1 = Lvectors_1.next()) {
                  var _b = __read(Lvectors_1_1.value, 2), vx = _b[0], vy = _b[1];
                  iterators.push(LshapeIterator({ x: x, y: y }, vx, vy));
              }
          }
          catch (e_1_1) { e_1 = { error: e_1_1 }; }
          finally {
              try {
                  if (Lvectors_1_1 && !Lvectors_1_1.done && (_a = Lvectors_1.return)) _a.call(Lvectors_1);
              }
              finally { if (e_1) throw e_1.error; }
          }
      }
  ...
  exports.boardIteratorsGenerator = boardIteratorsGenerator;
  ...
  function parseFen(fen) {
      ...
      var _b = __read(splitted, 6), pos = _b[0], nturn = _b[1], castling = _b[2], enpassant = _b[3], lastCatchNumber = _b[4], moveNumber = _b[5];
      ...
      pos
          ...
          var amount = splittedRow.reduce(function (prev, current) { return prev + (parseInt(current, 10) || 1); }, 0);
          if (amount > 8) {
              throw new Error('Invalid FEN position - too many pieces');
          }
          var x = 0;
          try {
              for (var splittedRow_1 = __values(splittedRow), splittedRow_1_1 = splittedRow_1.next(); !splittedRow_1_1.done; splittedRow_1_1 = splittedRow_1.next()) {
                  var token = splittedRow_1_1.value;
              ...
              try {
                  if (splittedRow_1_1 && !splittedRow_1_1.done && (_a = splittedRow_1.return)) _a.call(splittedRow_1);
              }
              finally { if (e_2) throw e_2.error; }
          }
      });
      var usedCastlingTokens = [];
      try {
          for (var _c = __values(castling.split('')), _d = _c.next(); !_d.done; _d = _c.next()) {
              var token = _d.value;
      ...
      return {
          ...
          pieces: pieces,
          possibleCastlings: possibleCastlings,
          turn: turn,
      };
  }
  ...
  var Piece_1 = require("./core/Piece");
  exports.Piece = Piece_1.Piece;
  var boardIterator_1 = require("./core/helpers/boardIterator");
  var fenParser_1 = require("./core/helpers/fenParser");
  var Point_1 = require("./core/helpers/Point");
  ...
  var ChessGame = (function (_super) {
      ...
      ChessGame.Point = Point_1.Point;
      ChessGame.Piece = Piece_1.Piece;
      ChessGame.parseFen = fenParser_1.parseFen;
      ChessGame.boardIteratorsGenerator = boardIterator_1.boardIteratorsGenerator;
      return ChessGame;
  }(Game_1.Game));
  exports.default = ChessGame;
  
  },{"./core/Game":2,"./core/Piece":4,"./core/helpers/Point":6,"./core/helpers/boardIterator":7,"./core/helpers/fenParser":8}],10:[function(require,module,exports){
  ...
  var css = "/*! chessboard.js v1.0.0 | (c) 2019 Chris Oakman | MIT License chessboardjs.com/license */\n.clearfix-7da63 {\n  clear: both;\n}\n.board-b72b1 {\n  border: 2px solid #404040;\n  box-sizing: content-box;\n}\n.square-55d63 {\n  float: left;\n  position: relative;\n  -webkit-touch-callout: none;\n  -webkit-user-select: none;\n  -khtml-user-select: none;\n  -moz-user-select: none;\n  -ms-user-select: none;\n  user-select: none;\n}\n.white-1e1d7 {\n  background-color: #f0d9b5;\n  color: #b58863;\n}\n.black-3c85d {\n  background-color: #b58863;\n  color: #f0d9b5;\n}\n.highlight1-32417,\n.highlight2-9c5d2 {\n  box-shadow: inset 0 0 3px 3px #ff0;\n}\n.notation-322f9 {\n  cursor: default;\n  font-family: \"Helvetica Neue\",Helvetica,Arial,sans-serif;\n  font-size: 14px;\n  position: absolute;\n}\n.alpha-d2270 {\n  bottom: 1px;\n  right: 3px;\n}\n.numeric-fc462 {\n  top: 2px;\n  left: 2px;\n}\nhtml,\nbody {\n  padding: 0;\n  margin: 0;\n  width: 100%;\n  height: 100%;\n  background: black;\n}\n.abs-center {\n  position: absolute;\n  left: 50%;\n  top: 50%;\n  transform: translate(-50%,-50%);\n}\n.legalMove {\n  -webkit-box-shadow: inset 0 0 3px 3px #7aff00;\n  box-shadow: inset 0 0 3px 3px #7aff00;\n}\n.message {\n  text-align: center;\n  margin: 8px;\n  font-size: 22px;\n  color: white;\n  font-style: italic;\n}\n"; (require("browserify-css").createStyle(css, { "href": "tests\\web\\app.css" }, { "insertAt": "bottom" })); module.exports = css;
  },{"browserify-css":11}],14:[function(require,module,exports){
  (function (global){
  "use strict";
  Object.defineProperty(exports, "__esModule", { value: true });
  require('./app.css');
  ...
  function removeSquareHighlights() {
      document.querySelectorAll('#board .legalMove').forEach(function ($square) {
          $square.classList.remove('legalMove');
      });
  }
  var only = '2r2k2/1B2pp2/8/6pP/1R2N3/8/2Q2P2/R3K2R w KQ - 1 2';
  var start = 'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1';
  var fen = only;
  var message = document.querySelector('.message');
  var engine = new index_1.default(fen);
  engine.on('checkMate', function (winner) {
      message.innerHTML = ("GameOver, winner => " + winner);
  });
  engine.on('staleMate', function () {
  ...
  var board = ChessBoard('board', {
      ...
      onDragStart: function (source, piece, position, orientation) {
          ...
          var possibleMoves = engine.select(source).getPossibleMoves();
          highlightSquares(possibleMoves.map(function (_a) {
              var x = _a.x, y = _a.y;
              return index_1.Point.getNotation(x, y);
          }));
      },
      onDrop: function (source, target) {
          ...
          try {
              var piece = engine.select(source);
              if (piece !== null) {
                  var move = piece.move(target);
              }
          }
          catch (err) {
              return 'snapback';
  ...
  },{"../../lib/index":9,"./app.css":13,"@chrisoakman/chessboardjs/dist/chessboard-1.0.0.min.js":10,"jquery":12}]},{},[14]);
